...
function render(state) {
  ...
  // local visibility at this truck's current zone (fall back to global)
  const localVis = (state.fog.zones && state.fog.zones[v.zone_type])
    ? state.fog.zones[v.zone_type].visibility_m
    : state.fog.visibility_m;

  // Sub-line
  let sub = `Visibility: ${localVis} m`;
  if (currentLevel === "YELLOW" && risk.other_id) sub = `Vehicle ahead – ${risk.gap_m} m`;
  ...
  const pos = state.positioning ? state.positioning[vid] : null;
  const gnssEl = document.getElementById("driver-gnss");
  if (pos) {
    gnssEl.textContent = pos.mode === "DEAD_RECKONING" ? "GNSS: DEAD RECKONING" : "GNSS: RTK ACTIVE";
    gnssEl.style.color = pos.mode === "DEAD_RECKONING" ? "#f1c40f" : "";
  }
}
